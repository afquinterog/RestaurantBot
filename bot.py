...
@bot.message_handler(regexp=r"^(Activar Plato|AP) ([0-9]+)$")
def mark_item_as_active(message):
    bot.send_chat_action(message.chat.id, 'typing')
    parts = re.match(r"^(Activar Plato|AP) ([0-9]+)$", message.text, re.IGNORECASE)
    index = int(parts[2])
    result = logic.mark_item_as_active(message.from_user.id, index)

    if (result):
        bot.reply_to(message, f"\U0001F372 Hemos activado el plato del menu")
    else:
